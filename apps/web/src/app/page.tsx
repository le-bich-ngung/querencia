...
function Typewriter({ text }: { text: string }) {
  ...
  return (
    <span>
      {displayed}
      {!done && <span style={{ borderRight: `2px solid ${SAGE}`, marginLeft: 1, animation: 'blink 0.7s step-end infinite' }}/>}
    </span>
  );
}
...
const LAW_QUOTES = [
  ...
  { text: "Laws are not masters but servants, and he rules them who obeys them.", author: "Ward Beecher" },
  { text: "Justice is the constant and perpetual will to allot to every man his due.", author: "Domitius Ulpianus" },
  { text: "In law, nothing is certain but the expense.", author: "Samuel Butler" },
  ...
  { text: "Laws too gentle are seldom obeyed; too severe, seldom executed.", author: "Benjamin Franklin" },
...
export default function HomePage() {
  ...
  const APPS = [
    { name: 'Cùi Bắp', emoji: '🌽', color: '#f59e0b', desc: 'Private messaging. No ads, no surveillance.', href: '/dashboard/cui-bap' },
    { name: 'Nope', emoji: '🌿', color: SAGE, desc: 'Real stories from real people.', href: '/dashboard/nope' },
    { name: 'LàNo', emoji: '🎧', color: '#8b5cf6', desc: 'Someone to listen. Without judgment.', href: '/dashboard/lano' },
    { name: 'Tools', emoji: '🔧', color: '#ef4444', desc: '44+ free browser-based tools.', href: '/tools' },
  ];

  const TOOLS = ['Image Editor','PDF → Word','Flashcards','Self-destruct Link','Screenshot Translate','Password Generator','Grammar Check','QR Generator','CV Builder','Text Summarize','Image Compress','Pomodoro Timer'];
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setLawVisible(false);
      setTimeout(() => {
        setLawIdx(i => (i + 1) % LAW_QUOTES.length);
        setLawVisible(true);
      }, 600);
    }, 10000);
    return () => clearInterval(t);
  ...
  return (
    <>
      ...
      <nav style={{
        ...
        background: 'rgba(8,10,8,0.8)', backdropFilter: 'blur(20px)',
        ...
        <div ref={langRef} style={{ position: 'relative' }}>
          <button onClick={() => setLangOpen(o => !o)} style={{
            ...
            display: 'flex', alignItems: 'center', gap: 4, color: 'rgba(240,239,235,0.6)',
          }}>
          ...
          {langOpen && (
            <div style={{
              ...
              background: '#1a1d1a', border: '1px solid rgba(255,255,255,0.08)',
              borderRadius: 10, padding: 4, zIndex: 300, minWidth: 150,
              ...
              {LOCALES.map(l => (
                <button key={l.code} onClick={() => { setLocale(l.code); setLangOpen(false); }} style={{
                  ...
                  background: locale === l.code ? 'rgba(74,124,89,0.15)' : 'transparent',
                  color: locale === l.code ? SAGE : 'rgba(240,239,235,0.7)',
                  fontWeight: locale === l.code ? 600 : 400, fontSize: '0.82rem',
        ...
        {session ? (
          <Link href="/dashboard/nope" style={{
            fontSize: '0.8rem', fontWeight: 600, color: '#fff',
            background: SAGE, borderRadius: 100, padding: '5px 14px',
            textDecoration: 'none',
          }}>Dashboard</Link>
        ) : (
          <>
            <Link href="/auth/login" style={{ fontSize: '0.8rem', color: 'rgba(240,239,235,0.6)', textDecoration: 'none', padding: '4px 8px' }}>
              {t('nav.signin')}
            </Link>
      ...
      <section style={{
        ...
        padding: '100px 24px 60px', overflow: 'hidden', textAlign: 'center',
      }}>
        {/* Ambient */}
        ...
        <div style={{ position: 'relative', animation: 'fadeIn 1s ease both' }}>
          ...
          <h1 style={{
            ...
            fontWeight: 400, letterSpacing: -4, lineHeight: 0.95,
            color: '#f0efeb', marginBottom: 8,
          }}>
            <span style={{ opacity: 0.35 }}>ueren</span>
            <span style={{ color: SAGE }}>cia</span>
          </h1>

          {/* Slogan */}
          <p style={{
            fontSize: '0.8rem', fontWeight: 500, letterSpacing: '0.2em',
            textTransform: 'uppercase', color: SAGE, opacity: 0.7, marginBottom: 48,
          }}>Tech and more</p>
          ...
          <div style={{
            ...
            color: 'rgba(240,239,235,0.45)', lineHeight: 1.7,
            maxWidth: 620, fontStyle: 'italic', minHeight: 80,
          }}>
      ...
      <section style={{ background: '#111814', padding: '40px 0', overflow: 'hidden' }}>
          ...
          <div className="ticker-inner">
            {[...TOOLS, ...TOOLS, ...TOOLS, ...TOOLS].map((tool, i) => (
              <span key={i} style={{
                ...
                borderRadius: 100, fontSize: '0.85rem',
        ...
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          <Link href="/tools" style={{ fontSize: '0.78rem', color: SAGE, textDecoration: 'none', opacity: 0.7, fontWeight: 500 }}>
            44+ free tools — explore all →
          </Link>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center', width: '100%' }}>
          ...
          <div style={{
            ...
            <blockquote style={{
              ...
              color: '#111', lineHeight: 1.55, letterSpacing: -0.3,
              marginBottom: 20,
            }}>
              "{currentQuote.text}"
            </blockquote>
            {currentQuote.author && (
              <p style={{ fontSize: '0.82rem', color: '#4a7c59', fontWeight: 600, letterSpacing: '0.04em' }}>
                — {currentQuote.author}
              </p>
            )}
          </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', width: '100%', position: 'relative' }}>
          <Reveal>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32 }}>
              ...
              <span style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(240,239,235,0.3)' }}>Law & Justice · From Read</span>
            </div>
          </Reveal>
          <div style={{
            opacity: lawVisible ? 1 : 0,
            transform: lawVisible ? 'translateY(0)' : 'translateY(10px)',
            transition: 'opacity 0.6s ease, transform 0.6s ease',
            minHeight: 140,
          }}>
            <blockquote style={{
              ...
              fontWeight: 400, fontStyle: 'italic',
              color: '#f0efeb', lineHeight: 1.5, letterSpacing: -0.3,
              marginBottom: 20, borderLeft: `3px solid ${SAGE}`, paddingLeft: 24,
            }}>
              "{currentLaw.text}"
            </blockquote>
            <p style={{ fontSize: '0.82rem', color: SAGE, fontWeight: 600, letterSpacing: '0.04em', paddingLeft: 24 }}>
              — {currentLaw.author}
            </p>
          </div>
        </div>
      ...
      <section style={{ background: '#fafaf8', padding: '80px 24px', minHeight: 400, display: 'flex', alignItems: 'center' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
          ...
          <div style={{
            ...
            transform: appVisible ? 'translateY(0)' : 'translateY(8px)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '1rem', color: '#666', marginTop: 8, fontFamily: 'DM Sans, sans-serif' }}>{currentApp.desc}</p>
              </div>
            </div>
            <Link href={currentApp.href} style={{
              display: 'inline-flex', alignItems: 'center', gap: 6,
              padding: '10px 22px', background: currentApp.color, color: '#fff',
              borderRadius: 100, textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600,
              fontFamily: 'DM Sans, sans-serif', transition: 'opacity 0.2s',
            }}>Explore {currentApp.name} →</Link>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 32 }}>
            {APPS.map((a, i) => (
              <div key={i} style={{
                width: appIdx === i ? 20 : 6, height: 6, borderRadius: 3,
                background: appIdx === i ? currentApp.color : '#ddd',
        ...
        <Reveal>
          <div style={{ position: 'relative' }}>
            ...
            <h2 style={{
              ...
              fontWeight: 400, letterSpacing: -2, lineHeight: 1.05,
              color: '#f0efeb', margin: '20px 0 16px',
            }}>
              One account.<br/>
              <span style={{ color: SAGE, fontStyle: 'italic' }}>Everything.</span>
            </h2>
            <p style={{ fontSize: '0.95rem', color: 'rgba(240,239,235,0.4)', marginBottom: 36, fontFamily: 'DM Sans, sans-serif' }}>
              Free, forever.
            </p>
            <Link href="/auth/register" style={{
              display: 'inline-block', padding: '14px 36px',
              background: SAGE, color: '#fff', borderRadius: 100,
