  ...
  return (
    <footer style={{ borderTop: '1px solid rgba(0,0,0,0.07)', background: '#fafaf8', padding: '14px 24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline' }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="22 22 72 72" width="12" height="12"
              style={{ flexShrink: 0, display: 'inline-block', verticalAlign: 'baseline', position: 'relative', top: '1px', marginRight: '0.3em' }}>
            ...
            {[
              ...
              return (
                <Link key={l.label} href={l.href}
                  style={{ fontSize: '0.72rem', color: '#bbb', textDecoration: 'none', transition: 'color 0.15s' }}
                  onMouseEnter={function(e) { e.currentTarget.style.color = SAGE; }}
                  onMouseLeave={function(e) { e.currentTarget.style.color = '#bbb'; }}
                >{l.label}</Link>
              );
            })}
          </div>
        ...
        {/* Right: tagline */}
        <span style={{ fontSize: '0.7rem', color: '#bbb', textAlign: 'right' }}>
          {t('footer.tagline')}
        </span>
